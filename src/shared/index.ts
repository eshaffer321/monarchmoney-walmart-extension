...
export async function withRetry<T>(fn: () => Promise<T>, retries = 3, backoffMs = 500): Promise<T> {
  ...
  for (let i = 0; i < retries; i++) {
    try { return await fn(); } catch (e: any) { lastErr = e; await delay(backoffMs * (i + 1)); }
  }
  throw lastErr;
}
...
export function safeJsonParse<T = any>(s: string, fallback: T | null = null): T | null { try { return JSON.parse(s); } catch { return fallback; } }
